...
func (e *e2e) enableAuditLoggingWithPath(logPath string) {
	...
	e.kubectlOperatorNS("rollout", "status", "ds", "spod", "--timeout", defaultWaitTimeout)
	e.waitInOperatorNSFor("condition=ready", "pod", "-l", "name=spod")
	// Additional wait for json-enricher to be fully ready
	time.Sleep(10 * time.Second)
}

func (e *e2e) enableAuditLoggingWithRotation(
	logPath string,
	maxSizeMB, maxBackups, maxAgeDays int,
) {
	e.logf(
		"Enable json-enricher with rotation: size=%dMB, backups=%d, age=%dd",
...
func (e *e2e) waitForAuditFileUpdate(nodeName, logPath string, timeout time.Duration) {
	...
	for {
		...
		cmd := fmt.Sprintf(
			`test -f %s && [ -s %s ] && echo "exists" || echo "missing"`,
			logPath, logPath,
		)
...
func (e *e2e) verifyAuditLogEntry(nodeName, logPath, expectedCmd, expectedPod string) bool {
	e.logf(
		"Verifying audit log for cmd '%s' in pod '%s' on node '%s'",
		expectedCmd,
		expectedPod,
		nodeName,
	)
...
func (e *e2e) verifyAuditLogEntryWithRetry(
	nodeName, logPath, expectedCmd, expectedPod string,
	maxRetries int,
) bool {
...
`, podName, containerName, profileName)
	...
	for i, cmd := range commands {
		...
		if i == 1 { // After touch
			...
			e.kubectl(
				...
				"sh",
				"-c",
				"ls -l "+workDir+"/AUDIT_LOG_TESTING_1",
			)
		}
	...
	case missedCount > 0 && missedCount <= 2:
		...
		e.logf(
			"WARNING: Known race condition - rapid consecutive commands may be missed due to OCPBUGS-62269",
		)
		e.logf(
			"PASS: Test passed with known limitation (%d/%d commands captured)",
			successCount,
			len(testCommands),
		)
	case missedCount > 2:
		...
		for _, cmd := range commands {
			...
			go func(namespace, podname string) {
				if _, err := e.kubectlCommand(
					...
					e.logf("Concurrent exec failed for %s/%s: %v", namespace, podname, err)
				}
			}(ns, pod)
		}
	}
...
`, podName, containerName, profileName)
	...
	reqOut := e.kubectl(
		...
		"sh",
		"-c",
		"printenv SPO_EXEC_REQUEST_UID || true",
	)
	...
	e.logf(
		"PASS: Basic functional validation via file audit log with requestUID correlation (%d/%d cmds)",
		found,
		len(commands),
	)
}
...
`, podName, containerName, profileName)
	...
	e.logf(
		"PASS: Seccomp logging profile validated for in-pod commands (%d/%d)",
		foundInPod,
		len(inPodCmds),
	)
}
...
`, podName, containerName, profileName)
	...
	for _, cmd := range testCommands {
		...
		reqUIDCmd := fmt.Sprintf(
			`grep -F 'sh -c %s' %s | head -n 1 | grep -o '"requestUID":"[^"]*"' || true`,
			cmd,
			logPath,
		)
