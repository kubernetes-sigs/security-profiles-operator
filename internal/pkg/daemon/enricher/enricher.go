...
func (e *Enricher) Run() error {
	...
	for l := range e.impl.Lines(tailFile) {
		...
		line := l.Text
		e.logger.Info("Got line: " + line)
		if !isAuditLine(line) {
			e.logger.Info("Not an audit line")
			continue
		}
		...
		e.logger.Info(fmt.Sprintf("Get container ID for PID: %d", auditLine.processID))
		...
		e.logger.Info("Get container info for: " + cID)
