...
func fetchTLSOptions(ctx context.Context, cfg *rest.Config) (
	...
	if isOpenShift {
		// Create a timeout context for fetching TLS configuration from OpenShift
		// Use a longer timeout since we're actually reading configuration data
		fetchCtx, fetchCancel := context.WithTimeout(ctx, 10*time.Second)
		defer fetchCancel()

		initialTLSProfile, err = tlspkg.FetchAPIServerTLSProfile(fetchCtx, preStartClient)
		if err != nil {
		...
		initialTLSAdherencePolicy, err = tlspkg.FetchAPIServerTLSAdherencePolicy(fetchCtx, preStartClient)
		if err != nil {
